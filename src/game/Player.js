...
export class Player {
  constructor(scene) {
    ...
    // Load 3D GLB Models exclusively
    this.loadGLTFModels();
      ...
      if (char.glbPath) {
        this.gltfLoader.load(
          ...
          (gltf) => {
            ...
            // Enable shadow casting and double-sided rendering for GLB mesh
            ...
            // Normalize GLB Model height to ~2.2 units
  ...
  // 🎮 EXCLUSIVE 3D GLB MODEL RUNNER MESH
  buildCharacterMesh() {
    ...
    if (gltfModel) {
      // 🚀 EXCLUSIVELY ATTACH YOUR REAL 3D GLB CHARACTER MODEL!
  ...
  // 🏃 DYNAMIC RUNNING MOTION APPLIED DIRECTLY TO YOUR 3D GLB CHARACTER MODELS!
  update(delta, runSpeed) {
    ...
    // 🏃 DYNAMIC RUNNING MOTION APPLIED TO YOUR REAL GLB MODELS (czar.glb, illkery.glb, modex.glb)
    if (this.isJetpackActive) {
      // 🚀 Jetpack Superman Flight
      this.bodyGroup.position.y = 0;
      ...
      // 🏃 Jump Tilt
      this.bodyGroup.position.y = 0;
      this.bodyGroup.rotation.x = -0.18;
      this.bodyGroup.rotation.y = 0;
      this.bodyGroup.rotation.z = 0;
      this.bodyGroup.scale.set(1, 1, 1);
    } else {
      // 🏃 REAL 3D GLB RUNNING MOTION: Footfall Stride Bounce, Hip Twist & Running Sway!
      const bounce = Math.abs(Math.sin(this.animTime * 2.8)) * 0.22;
      const strideTwist = Math.cos(this.animTime * 2.8) * 0.28; // Dynamic 3D Stride Twist
      const runningSway = Math.sin(this.animTime * 2.8) * 0.12; // Dynamic Running Lean Sway

      this.bodyGroup.position.y = bounce;
      this.bodyGroup.rotation.y = strideTwist; // Stride Twist applied directly to your GLB model!
      this.bodyGroup.rotation.z = runningSway; // Running Sway applied directly to your GLB model!
      this.bodyGroup.rotation.x = 0.08; // Forward Running Tilt!
      this.bodyGroup.scale.set(1, 1, 1);
    }
